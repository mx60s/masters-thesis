...
def _load_train_test_data(
    ...
    if 'conv5_block2_out' in config['output_layer']:
        print("Sampling partial")
        model_name = config['model_name']
        output_layer = config['output_layer']
        sampled_indices = np.load(f'results/sample_indices_{model_name}_{output_layer}.npy')

        model_reps = model_reps[:, sampled_indices]

    model_reps = np.reshape(model_reps, (-1, model_reps.shape[2]))
    #print("model reps shape before pruning", model_reps.shape)

    #nulls = np.where(np.all(model_reps == 0, axis=1))[0]
    #model_reps = np.delete(model_reps, nulls, axis=0)
    #targets_true = np.delete(targets_true, nulls, axis=0)
    
    #print("model reps shape after pruning", model_reps.shape)
...
def normalize_error(error):
    """
    Normalize the error with respect to the environment size.
    Input error is assumed to be MSE (squared distance).
    """
    return np.sqrt(error) / 24

def normalize_rotation(error):
    """
    Normalize the error with respect to the environment size.
    Input error is assumed to be MSE (squared distance).
    """
    return np.sqrt(error) / 50000
...
def _fit_decoding_model(
    ...
    #x_mean = -0.0618798
    #x_std = 11.80241442938637
    #z_mean = -0.0458212
    #z_std = 18.412212584658434

    #y_train[:, 0] = (y_train[:, 0] - x_mean) / x_std
    #y_train[:, 1] = (y_train[:, 1] - z_mean) / z_std
    ...
    #y_pred[:, 0] = (y_pred[:, 0] * x_std) + x_mean
    #y_pred[:, 1] = (y_pred[:, 1] * z_std) + z_mean

    # compute errors ------------------------------------
    #print("Predicted")
    #pred = np.array2string(y_pred[:50, :2], precision=4, suppress_small=True)
    #print(pred)
    #print("Actual")
    #print(y_test[:50, :2])
    ...
    mse_loc_normalized = normalize_error(mse_loc)
    mse_rot = np.mean(per_loc_mse_rot_samples)
    mse_rot_normalized = normalize_rotation(mse_rot)
    
    
    # Normalize confidence intervals
    ci_loc = stats.bootstrap((per_loc_mse_loc_samples,), np.mean).confidence_interval
    ci_loc_normalized = (normalize_error(ci_loc[0]), normalize_error(ci_loc[1]))
    ci_rot = stats.bootstrap((per_loc_mse_rot_samples,), np.mean).confidence_interval
    ci_rot_normalized = (normalize_rotation(ci_rot[0]), normalize_rotation(ci_rot[1]))
    ...
    mid_loc = np.array(
        [(config['env_x_max'] + config['env_x_min']) / 2,
        (config['env_y_max']+config['env_y_min']) / 2]
    )
    print("mid location", mid_loc)
    
    mid_rot = np.array([config['n_rotations']//2]) #maybe this is the issue
    print("mid location", mid_rot)
    ...
    baseline_predict_mid_mse_loc_normalized = normalize_error(baseline_predict_mid_mse_loc)
    baseline_predict_random_mse_loc_normalized = normalize_error(baseline_predict_random_mse_loc)

    baseline_predict_mid_mse_rot_normalized = normalize_rotation(baseline_predict_mid_mse_rot)
    baseline_predict_random_mse_rot_normalized = normalize_rotation(baseline_predict_random_mse_rot)

    return mse_loc_normalized, mse_rot_normalized, ci_loc_normalized, ci_rot_normalized, \
        decoding_model.coef_, decoding_model.intercept_, \
        baseline_predict_mid_mse_loc_normalized, baseline_predict_mid_mse_rot_normalized, \
        baseline_predict_random_mse_loc_normalized, baseline_predict_random_mse_rot_normalized
...
def _single_env_decoding_error(
    ...
    print("Starting _single_env_decoding_error")
    ...
    if fresh_start:
        ...
        if config['model_name'] == 'none' or 'predictive-coding' in config['model_name'] or 'autoencoder' in config['model_name']:
            model = None
        ...
        print("Loading positions from numpy")
        #if 'resnet' in config['model_name']:
        env = config['unity_env']
        targets_true = np.load(f'data/minecraft/{env}_pos.npy')
        ...
        print(f"Saving at {results_path}/res.npy")
            ...
            for feature_selection in feature_selections:
                for decoding_model_choice in decoding_model_choices:
                    ...
                    for i, error_type in enumerate(error_types):
                        ...
                        if error_type == 'loc':
                            axes.set_ylabel('Normalized Decoding Error')
                            axes.set_title('Normalized Location Decoding')
                        else:
                            axes.set_ylabel('Normalized Decoding Error')
                            axes.set_title('Normalized Direction Decoding')
...
if __name__ == '__main__':
    ...
    # =================================================================== #
    TF_NUM_INTRAOP_THREADS = 7
    CPU_NUM_PROCESSES = 5
    envs = ['small_world_r17'] #'biggest_world_r17']
    movement_modes = ['2d']
    sampling_rates = [0.3]
    random_seeds = [42]
    model_names = [
    ...
    if perform_lesion:
        ...
        metrics = ['directioness', 'maxvalueinclusters', 'numclusters']#, 'entiremapmean'] #, 'borderness', 'entiremapmean']                # if reference_experiment=='unit_chart', metric='borderness|..', else 'coef'
        thrs = ['0']                            # if metric=='coef', thr='thr', else '0'
        ranks = ['random', 'top']#, 'random']                         # 'top|random'
