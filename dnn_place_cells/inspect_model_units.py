...
One major edit was reported here: https://github.com/don-tpanic/Space/issues/1 
This fix changes several of the pie charts that were included in Luo et al. but leaves the lesioning studies intact
Figures in the published thesis include this fix.
'''
...
import old_unit_metric_computers as oumc
...
def _single_model_reps(config):
    ...
    if os.path.exists(model_reps_fname):
        ...
        print('early model reps', model_reps.shape)
        ...
        # I mean, you do this twice for some reason idk
...
def _single_env_produce_unit_chart(
    ...
    # load model outputs
    model_reps = _single_model_reps(config) # in shape num_locs, num_rots, num_features

    n_rotations = config['n_rotations']
    num_units = model_reps.shape[1]

    model_reps = np.reshape(model_reps, 
                                (model_reps.shape[0] // n_rotations, 
                                 n_rotations, model_reps.shape[1]))
    ...
    if sample_partial:
        ...
        if num_units > 8000:
            sampled_indices = np.random.choice(num_units, 8000, replace=False)
            model_reps = model_reps[:, :, sampled_indices]
            logging.info(f'Sampled 8000 units out of {num_units}')
            #model_reps = np.reshape(model_reps, 
            #                    (model_reps.shape[0] // n_rotations, 
            #                     n_rotations, model_reps.shape[1]))
            ...
            np.save(f'/home/mag/Space/results/sample_indices_{model_name}_{output_layer}.npy', sampled_indices)
            
        else:
            logging.info(f'Less than 8000 units ({num_units}), using all units')

    if config['unity_env'] == 'biggest_world_r17' and 'resnet' in config['model_name']:
        locations = np.load('data/minecraft/biggest_world_r17_pos.npy')
        x_range = np.arange(env_x_min * multiplier, env_x_max * multiplier + 1)
        y_range = np.arange(env_y_min * multiplier, env_y_max * multiplier + 1)
    ...
    if 'vit' in config['model_name'] or 'post_pred' in config['output_layer']:
        print('[Check] transformer-based model detected, relu model_reps.')
        model_reps[model_reps < 0] = 0
        print(np.any(model_reps < 0))
    ...
    unit_chart_info = np.zeros(
        (model_reps.shape[2], len(charted_info)), dtype=object # num_features, num_charted_info
    )
    model_reps_summed = np.sum( # num_locs, num_rots, num_features
        model_reps, axis=1, keepdims=True
    )
    
    if env_x_min != -20:
        pixel_min_threshold = 20
    else:
        pixel_min_threshold = 90
    
    for unit_index in range(model_reps_summed.shape[2]):
        logging.info(f'[Charting] unit_index: {unit_index} / {model_reps_summed.shape[2]}')
        ...
        if movement_mode == '2d':
            ...
            if umc._is_dead_unit(heatmap):
                ...
                #print(f'Unit {unit_index} active')
                ...
                if use_old_umc:
                    ...
                    mean_value_in_clusters, var_value_in_clusters, \
                        bounds_heatmap, mean_cluster_angle = \
                            oumc._compute_single_heatmap_fields_info(
                                heatmap=heatmap,
                                pixel_min_threshold=pixel_min_threshold,
                                pixel_max_threshold=\
                    ...
                    num_clusters, num_pixels_in_clusters, max_value_in_clusters, \
                        mean_value_in_clusters, var_value_in_clusters, \
                            bounds_heatmap = \
                                umc._compute_single_heatmap_fields_info(
                                    model_reps[:, :, unit_index], # now passing in the unsummed model_reps, so hd is preserved
                                    unit_index,
                                    env_x_max,
                                    env_y_max,
                                    env_x_min,
                                    env_y_min,
                                    multiplier,
                                    pixel_min_threshold= pixel_min_threshold, # 10,
                                    pixel_max_threshold=\
                                    int(heatmap.shape[0]*heatmap.shape[1]*0.5)
                                )
                ...
                # I commented this out but it doesn't appear to affect anything?
                # 2. gridness
                ...
                # 3. borderness
                if use_old_umc:
                    border_score = oumc._compute_single_heatmap_border_scores(heatmap)
                else:
                ...
                if use_old_umc:
                    directional_score, per_rotation_vector_length = \
                        oumc._compute_single_heatmap_directional_scores(
...
def _single_env_viz_unit_chart(
    ...
    for unit_index in tqdm(range(unit_chart_info.shape[0])):
                        ...
                        for feature_selection in feature_selections:
                            for decoding_model_choice in decoding_model_choices:
                                for random_seed in random_seeds:
                                    ...
                                    single_entry['filterings'] = filterings
                                    single_entry['sample_partial'] = sample_partial
                                    args_list.append(single_entry)
