...
class EnvironmentGenerator(IterableDataset):
    ...
    def generate_path(self, momentum_coeff=None, dir_inertia_coeff=None):
        ...
        # it should be opposite lol sorry
        ...
        random_turn = np.random.normal(0, sigma, [num_samples+1]).astype(np.float32) # functionally with dt this ends up being like 60 deg var
        random_vel = np.random.normal(0, b, [num_samples+1]).astype(np.float32)
        random_vel = np.abs(random_vel)
        #random_vel = np.random.rayleigh(b, [num_samples+1]).astype(np.float32)
    ...
    def __next__(self):
        ...
        #state = np.concatenate((position, move_dir), axis=1)
