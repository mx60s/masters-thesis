...
class EnvironmentGenerator(IterableDataset):
    def __init__(
        ...
        while True:
            j, i = np.random.randint(0, 41), np.random.randint(0, 65)
            z = i - 29.5
            x = -j + 19.5
    ...
    def follow_path(self, path, alpha=1, beta=0.6):
        ...
        if ntics > 600000:  # why 240?
            ...
            self.expected_z = z
            self.expected_yaw = yaw
            self.require_yaw_change = self.curr_yaw != yaw
            self.require_move = self.curr_x != x or self.curr_z != z
            self.prev_state, error = self.wait_next_state()
            self.path = self.generate_path()
            self.best_index = 0
            self.start_time = time.time()
