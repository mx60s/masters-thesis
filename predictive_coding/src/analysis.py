# Original Gornet and Thomson code, not used here
...
class PositionDecoder:
    ...
    def train(self, latents: Latents):
        inputs = torch.from_numpy(latents.latents).float().to(self.device)
        positions = torch.from_numpy(latents.positions).float().to(self.device) / 30
        for _ in (pbar := tqdm(range(8000))):
            ...
            for it, idx in enumerate(batch_idx):
                ...
                print('positions', positions[idx, :2])
                pred = self.net(batch)
                print('pred', pred.cpu())
    ...
    def error(self, latents: Latents) -> np.ndarray:
        ...
        error = np.linalg.norm((latents.positions[:, :2] - pred.numpy()), axis=1)
